...
struct List
{
	...
	void Insert(Element* newElement, int position)
	{
		if (position > 0)
		{
			...
			if (Count != position)
			{
				secondElement->previousElement = newElement;
			}
		...
		else
		{
			if (Count != 0)
			{
				...
				temp->previousElement = firstElement;
			}
			else
			{
				firstElement = newElement;
		...
		lastElement = GetElementWithNumber(Count - 1);
	}
	...
	void PrintFromTheEnd()
	{
		...
		for (int i = 0; i < Count; i++)
		{
			element = GetElementFromTheEndWithNumber(i);
			cout << element->a.ToString();
		}
		cout << '\n';
	}
	...
	void Remove(int position, int count = 1)
	{
		...
		{
			GetElementWithNumber(position + count - 1)->previousElement = NULL;
	...
	Element* GetElementFromTheEndWithNumber(int numberOfElement)
	{
	...
	void SearchByCriterionFromTheEnd(Criterion criterion)
	{
		if (criterion == name)
		{
			cout << "¬ведите название сорта зерна дл€ поиска:\n";
			...
			for (int i = 0; i < Count; i++)
			{
				mon mo = GetElementFromTheEndWithNumber(i)->a;
				int result = CompareStrings(mo.name, word);
				if (result == 0)
				{
					cout << mo.ToString();
